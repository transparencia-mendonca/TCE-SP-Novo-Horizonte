...
const DBN="tcesp-novo-horizonte-v7",STORE="dados",KEY="base";
...
function tceExpenseUrl(year){return `https://transparencia.tce.sp.gov.br/municipio/novo-horizonte/${year}/despesas`}
...
const MUNICIPIO_API="novo-horizonte";
...
async function fetchPermanentYear(y,force=false,manifestOverride=null){
  ...
  const file=item.arquivo||item.file||`data-${y}.json`;
  const bust=encodeURIComponent((manifest.version||cfg.version||window.TCESP_BUILD_ID||"baseline")+`-${y}`);
  $("#status").textContent=`Carregando ${y} da base permanente…`;
...
(async()=>{
  let initError=null;
  try{
    // Novo Horizonte tem uma base muito maior que as cidades anteriores.
    // Para evitar travamento/estouro de memória no Safari móvel, carregamos apenas
    // o exercício mais recente na abertura e os demais sob demanda pelo seletor.
